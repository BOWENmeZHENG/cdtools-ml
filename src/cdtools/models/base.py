...
class CDIModel(t.nn.Module):
            ...
            for inputs, patterns in data_loader:
                ...
                def closure():
                    ...
                    for inp, pats in zip(input_chunks, pattern_chunks):
                        ...
                        # Run the simulation
                        sim_patterns, exit_wave = self.forward(*inp)
                ...
                # Always update obj and probe parameters
    ...
    def Adam_optimize(
        ...
        ptycho_optimizer : torch.optim.Optimizer
            The optimizer for ptycho parameters
        ml_optimizer : torch.optim.Optimizer
            The optimizer for ML parameters
        batch_size : int
        ...
        self.training_history += (
            f'Planning {iterations} epochs of Adam, with batch size = {batch_size}, '
            f'regularization_factor = {regularization_factor}, and schedule = {schedule}.\n'
        )
    ...
    # def set_exit_wave_saving(self, epochs, save_dir=None, store_in_list=True):
    #     """Configure exit wave saving at specified epochs
        
    #     Parameters
    #     ----------
    #     epochs : list of int
    #         List of epoch numbers at which to save exit waves
    #     save_dir : str, optional
    #         Directory to save exit wave files. If None, saves in current directory
    #     store_in_list : bool, optional
    #         Whether to store exit waves in self.exit_wave_list. Default is True
    #     """
    #     self.save_exit_wave_epochs = epochs
    #     self.exit_wave_save_dir = save_dir
        
    #     if store_in_list:
    #         self.exit_wave_list = []  # Reset the list

    #     if save_dir:
    #         os.makedirs(save_dir, exist_ok=True)

    # def _save_exit_wave(self, exit_wave):
    #     full_numpy_filename = f"exit_wave_epoch_{self.epoch:04d}_all_patterns.npy"
    #     full_numpy_filepath = os.path.join(self.exit_wave_save_dir, full_numpy_filename)
    #     np.save(full_numpy_filepath, exit_wave.detach().cpu().numpy())
